...
new BitacoraDAO().registrarActividad(usuario.getId(), "Ingresó al Quiz de Enojo");
%>
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <title>Quiz de Enojo - EmotiKids</title>
    ...
    <style>
        ...
        .opcion-respuesta:hover { background-color: #FFADAD; color: white; }
        .opcion-respuesta input[type="radio"] { transform: scale(1.8); margin-right: 15px; }
        .pregunta-caja { background-color: #FFF2F2; border: 4px solid #FFADAD; border-radius: 20px; padding: 25px; margin-bottom: 30px; text-align: left; }
    </style>
</head>
<body>
<main>
    ...
    <div class="form-container" style="max-width: 850px; text-align: center;">
        <h2 style="font-size: 40px; color: #E5383B;">😡 Quiz del Enojo 😡</h2>
        <p style="font-size: 20px; font-weight: bold;">Escucha la pregunta y escoge la respuesta correcta.</p>
        <hr style="border: 2px dashed #FFADAD; margin: 20px 0;">

        <div class="pregunta-caja">
            <h3 id="preg1" style="display: inline-block; font-size: 26px; color: #4A4E69;">1. ¿Cuándo puede aparecer el enojo?</h3>
            <button type="button" onclick="leerTexto('preg1')" style="background:none; border:none; font-size:28px; cursor:pointer;">🔊</button><br><br>
            <label class="opcion-respuesta"><input type="radio" name="p1" value="1"> Cuando algo no sale como esperamos 🧩</label>
            <label class="opcion-respuesta"><input type="radio" name="p1" value="0"> Cuando dormimos 😴</label>
            <label class="opcion-respuesta"><input type="radio" name="p1" value="0"> Cuando estamos felices 😄</label>
        </div>

        <div class="pregunta-caja">
            <h3 id="preg2" style="display: inline-block; font-size: 26px; color: #4A4E69;">2. ¿Qué podemos hacer para calmarnos?</h3>
            <button type="button" onclick="leerTexto('preg2')" style="background:none; border:none; font-size:28px; cursor:pointer;">🔊</button><br><br>
            <label class="opcion-respuesta"><input type="radio" name="p2" value="1"> Respirar profundo 🧘</label>
            <label class="opcion-respuesta"><input type="radio" name="p2" value="0"> Gritar más fuerte 🗣️</label>
            <label class="opcion-respuesta"><input type="radio" name="p2" value="0"> Romper cosas 💥</label>
        </div>

        <div class="pregunta-caja">
            <h3 id="preg3" style="display: inline-block; font-size: 26px; color: #4A4E69;">3. ¿Cuál imagen representa enojo?</h3>
            ...
            <label class="opcion-respuesta"><input type="radio" name="p3" value="1"> 😡 Cara enojada</label>
            <label class="opcion-respuesta"><input type="radio" name="p3" value="0"> 😨 Cara con miedo</label>
        </div>
        ...
        <h3 id="resultado" style="font-size: 28px; color: #E5383B; margin-top: 15px;"></h3>
        <br><a class="btn-volver" href="<%=request.getContextPath()%>/actividades/actividades.jsp">🎮 Volver a Actividades</a>
    </div>
    <%@ include file="/componentes/footer.jsp" %>
</main>
<script>
function leerTexto(id){ let msj = new SpeechSynthesisUtterance(document.getElementById(id).innerText); msj.lang = "es-ES"; window.speechSynthesis.cancel(); window.speechSynthesis.speak(msj); }
function calificarQuiz() {
    ...
    if (p1 && p1.value == 1) total++; else errores += "<p style='color:red;'>1: Respuesta correcta: Cuando algo no sale como esperamos 🧩</p>";
    if (p2 && p2.value == 1) total++; else errores += "<p style='color:red;'>2: Respuesta correcta: Respirar profundo 🧘</p>";
    if (p3 && p3.value == 1) total++; else errores += "<p style='color:red;'>3: Respuesta correcta: 😡 Cara enojada</p>";
    document.getElementById("estrellas").innerHTML = "⭐".repeat(total) + "☆".repeat(3 - total);
    let msg = document.getElementById("resultado");
    if (total == 3) { msg.innerHTML = "🎉 ¡Excelente! Conoces muy bien el enojo."; document.getElementById("detalleErrores").innerHTML = ""; } 
