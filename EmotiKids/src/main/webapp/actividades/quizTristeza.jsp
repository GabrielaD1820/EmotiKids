...
new BitacoraDAO().registrarActividad(usuario.getId(), "Ingresó al Quiz de Tristeza");
%>
<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <title>Quiz de Tristeza - EmotiKids</title>
    ...
    <style>
        ...
        .opcion-respuesta:hover { background-color: #A0C4FF; color: white; }
        .opcion-respuesta input[type="radio"] { transform: scale(1.8); margin-right: 15px; }
        .pregunta-caja { background-color: #F2F7FF; border: 4px solid #A0C4FF; border-radius: 20px; padding: 25px; margin-bottom: 30px; text-align: left; }
    </style>
</head>
<body>
<main>
    ...
    <div class="form-container" style="max-width: 850px; text-align: center;">
        <h2 style="font-size: 40px; color: #6495ED;">😢 Quiz de la Tristeza 😢</h2>
        <p style="font-size: 20px; font-weight: bold;">Escucha la pregunta y escoge la respuesta correcta.</p>
        <hr style="border: 2px dashed #A0C4FF; margin: 20px 0;">

        <div class="pregunta-caja">
            <h3 id="preg1" style="display: inline-block; font-size: 26px; color: #4A4E69;">1. ¿Qué puede hacernos sentir tristeza?</h3>
            <button type="button" onclick="leerTexto('preg1')" style="background:none; border:none; font-size:28px; cursor:pointer;">🔊</button><br><br>
            <label class="opcion-respuesta"><input type="radio" name="p1" value="0"> Jugar con amigos ⚽</label>
            <label class="opcion-respuesta"><input type="radio" name="p1" value="1"> Perder un juguete favorito 🧸</label>
            <label class="opcion-respuesta"><input type="radio" name="p1" value="0"> Comer un helado 🍦</label>
        </div>

        <div class="pregunta-caja">
            <h3 id="preg2" style="display: inline-block; font-size: 26px; color: #4A4E69;">2. ¿Qué podemos hacer cuando estamos tristes?</h3>
            <button type="button" onclick="leerTexto('preg2')" style="background:none; border:none; font-size:28px; cursor:pointer;">🔊</button><br><br>
            <label class="opcion-respuesta"><input type="radio" name="p2" value="1"> Hablar con alguien de confianza 🫂</label>
            <label class="opcion-respuesta"><input type="radio" name="p2" value="0"> Romper cosas 💥</label>
            <label class="opcion-respuesta"><input type="radio" name="p2" value="0"> Gritar a todos 🗣️</label>
        </div>

        <div class="pregunta-caja">
            <h3 id="preg3" style="display: inline-block; font-size: 26px; color: #4A4E69;">3. ¿Cuál imagen representa tristeza?</h3>
            ...
            <label class="opcion-respuesta"><input type="radio" name="p3" value="1"> 😢 Cara triste</label>
        ...
        <h3 id="resultado" style="font-size: 28px; color: #6495ED; margin-top: 15px;"></h3>
...
function calificarQuiz() {
    ...
    if (p1 && p1.value == 1) total++; else errores += "<p style='color:red;'>1: Respuesta correcta: Perder un juguete favorito 🧸</p>";
    if (p2 && p2.value == 1) total++; else errores += "<p style='color:red;'>2: Respuesta correcta: Hablar con alguien de confianza 🫂</p>";
    if (p3 && p3.value == 1) total++; else errores += "<p style='color:red;'>3: Respuesta correcta: 😢 Cara triste</p>";
    document.getElementById("estrellas").innerHTML = "⭐".repeat(total) + "☆".repeat(3 - total);
    let msg = document.getElementById("resultado");
    if (total == 3) { msg.innerHTML = "🎉 ¡Excelente! Conoces muy bien la tristeza."; document.getElementById("detalleErrores").innerHTML = ""; } 
